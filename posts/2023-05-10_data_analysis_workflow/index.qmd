...
It seems the latter interpretation is the one that most folks tend to think they are making and definitely want to be making. And that all stems from the fact that the Bayesian workflow is building a density distribution of parameter values. After you have built the distribution, you can just summarise it in a more straightforward manner using tools from descriptive statistics to summarise the shape and extent of the distribution.

In contrast, the frequentist interval is a flat line, such that all you can say is that 95% of an infinite number of 95% intervals will contain the true parameter. But that parameter could be anywhere along the flat line. It is no more likely to be in the middle as at the edge of the line. A density distribution is different, by definition, because it has a particular shape which, for example, may have the highest density in the middle of the distribution. 

#### It is the structure of the model that matters, not the p value or Bayes factor

The main approach is motivated by building multi-level regression models that try to estimate parameters that reflect reality as much as possible. As such, it is the integrity and structure of the model and the assumptions that the model embodies that takes centre stage, rather than the bright lights of a p-value or Bayes factor value.

One major benefit of this approach is the differnece to traditional stats training. Rather than learning an almost infinite list of “tests” like your standard undergraduate or postgraduate stats class, it is a more general and flexible approach that is applicable in many different contexts. 
