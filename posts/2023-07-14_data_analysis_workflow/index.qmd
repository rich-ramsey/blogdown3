...
If I have confused things, then that's a shame. Such is life. Please send me a postcard with constructive feedback.
...
My background and training is in experimental psychology and cognitive neuroscience and my main research methods and statistics training will be very familiar to those from a psychology background. Each week we learned about a different statistical "test". And each week we were given some data, which was suitable for a particular test. Week 1 starts with a t-test and you head towards more complex ANOVAs as the weeks move on. A version of this type of training seems to be the cornerstone of most undergraduate and graduate statistics courses in psychology across the globe.

All you ever needed to do was pick the right test for the particular type of data and pray to the datagods that the p value would be \< 0.05. If p was \> 0.05, but still close (e.g., \< 0.1), then say something vague like "trending towards significance". If p value was \> 0.1 then just pretend that that means there is no real effect.
...
```{r, echo=FALSE, fig.align='left', out.width="75%", out.height="75%"}
...
As you can see, these four parameters vary in their highest density (what you might take for a point estimate) and in their spread or interval. Two are centred on zero, two are centred at 2. Two have relatively broad distributions whereas two have relatively narrow distributions.

In each case, you could then describe the proportion of values that span a particular range or quantile interval. e.g., for parameter 1, values as low as -2 and as high as 2 are compatible with your model estimates.

Then consider what this might mean in relation to your pre-registered hypotheses. Let's say for simplicity, that for all four parameters, we pre-registered that each would need to be positive to support our hypotheses. Parameters 1 and 2 would not show clear support the hypothesis, but parameter 3 and 4 would. Parameters 2 and 4 would be narrower estimates and more precise than parameters 1 and 3. Therefore, we may initially be more confident about drawing inferences from parameters 1 and 3 than 2 and 4.
...
Ok, that's fine and fairly clear-cut, as far as data from one experiment can get you. But what about grey areas or distributions that are a little more indeterminate in relation to our hypotheses? Take a look at the following.
...
All four have the same spread. And all four have a majority of positive values. However, they vary in the strength with which they would support our hypothesis. Some have a healthy number of negative values in the 95% quantile interval (parameters 5 and 6) and some have a lower bound close to zero (parameters 7 and 8). Therefore, none of these provide overwhelming support for our hypothesis, at least not on their own as it stands from one single study. With that being said, parameter 7 and 8 are more compatible with our hypothesis than parameter 5 and 6. That much is clear and obvious.

But, overall, more experiments and data would be needed. This is true in all cases in my view, but especially in this particular case because of the indeterminate nature of the distributions. An obvious next step would be to try to increase the precision of the estimate by collecting more data and see if a replication can make the estimates more precise and support the original findings.
...
set.seed(123)
d4 <-  tibble(pid = 1:300,
              parameter5 = rnorm(300, mean = 0.5, sd = 0.5),
              parameter6 = rnorm(300, mean = 1, sd = 0.5),
              parameter7 = rnorm(300, mean = 1.5, sd = 0.5),
              parameter8 = rnorm(300, mean = 2, sd = 0.5)) %>% 
  pivot_longer(-pid,
               names_to = "term",
               values_to = "value") %>% 
  mutate(pid = factor(pid),
...
set.seed(123)
d5 <-  tibble(pid = 1:500,
              parameter5 = rnorm(500, mean = 0.5, sd = 0.25),
              parameter6 = rnorm(500, mean = 1, sd = 0.25),
              parameter7 = rnorm(500, mean = 1.5, sd = 0.25),
              parameter8 = rnorm(500, mean = 2, sd = 0.25)) %>% 
  pivot_longer(-pid,
               names_to = "term",
               values_to = "value") %>% 
  mutate(pid = factor(pid),
