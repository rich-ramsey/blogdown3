...
> Adding parameter 4 \<insert variable name here e.g., gender \| age \| factor A vs. B\> made a clear improvement to predictive accuracy. This means that when this term was included, the model made more accurate predictions of the unseen data. This supports our hypothesis that <refer to the stated hypothesis>. In contrast, when adding parameters 6 and 7 to the model, there was no added benfit to out-of-sample predictive accuracy. Such results, on their own, do not support our hypothesis that <refer to the stated hypothesis>.
...
3.  The front-end software package for building models (brms) will be familiar to many people with a psychology background because uses the same syntax as the very popular linear mixed-effects package (lme4).

Cons

1.  Model building takes more time (hours, days, weeks, months?) and needs greater computer power to make it feasible[^3].
...
[^3]: it is worth noting that Bayesian analyses were only made possible for regular folks in the mid-1990s with the advent of more powerful desktop computers
