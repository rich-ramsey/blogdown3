...
### What's my problem with the p values and Bayes factors? ###

In principle, I have no problem with either the p value or the Bayes factor, *if used appropriately*. They are both just redundant in the workflow that I outlined above. In my lab's work and the statistical textbooks that I've read, I have not seen a situation yet where they add any value to the calculation and evaluation of parameters in a multi-level regression model. 

To make matters even less appealing, since its inception, the p value has been butchered so badly that it seems to have lost all recognition of itself and its rather restricted use-cases. The situation is so pervasive that folks teaching how to use the p value seem to mis-using it (REF). Very sensible people have come to the p values rescue by outlining how you should use it. That's fine and good. Another approach, however, is to let the p value quietly die on the slag heap of scientific mis-adventures and start again with a fresh perspective. And I prefer the latter approach.
...
Now, I am sure that there are many, **many** more qualified people who can tell me all the reasons why such metrics can add value and in which specific situations they do so. And that's great. Science is best-served by a diverse and broad base. But, given the designs and datasets that my lab uses, which span approaches from experimental psychology to social and cognitive neuroscience, they just don't add any value, but they come with associated costs. So I put them in a big bin (e.g., a skip or a dumpster, depending on your British-American leanings).

There is one major consequence of doing so. There are no bright lines for inference. There are no categorical decision points or inferential cliffs to fall off at p < 0.05. And there are no pre-set benchmarks for determining how "evidential" a Bayes factor is, in terms of . And they are all good things to avoid. Instead, dare I even say it as a professional scientist, you eventually need to use evaluative language to consider what your results mean. Lordy lordy. Whatever next? 
